...
:::info Endpoint naming
`/api/metacode` is the canonical, brand-neutral endpoint name (works for ZIP codes, postal codes, postcodes, PLZ, and equivalent). The original name `/api/metazip` continues to work indefinitely as a legacy alias — see the [/api/metazip page](./metazip) for details. New integrations should use `/api/metacode`.
:::
